/*
 * render.cpp - turn Donkey Kong's 256x224 native frame upright on the 240x280 panel.
 *
...
#define NUM_FB 2
#define ROT_W DK_FB_H          /* 224: the upright picture's width  */
#define ROT_H DK_FB_W          /* 256: and its height               */

static uint8_t *fbs[NUM_FB];
static QueueHandle_t free_q, frame_q;
static uint16_t *chunk;
static uint16_t pal_swapped[DK_PALETTE_SIZE];
...
static void present(const uint8_t *fb)
{
    ...
    dk_palette(pal);
    for (int i = 0; i < DK_PALETTE_SIZE; i++) pal_swapped[i] = (uint16_t)((pal[i] >> 8) | (pal[i] << 8));
    ...
    for (int row = 0; row < DISPLAY_HEIGHT; row += ROWS_PER_CHUNK) {
        ...
        for (int r = 0; r < rows; r++) {
            ...
            for (int px = 0; px < DISPLAY_WIDTH; px++) {
                int rx = x_map[px];
                *dst++ = pal_swapped[src[(DK_FB_H - 1 - rx) * DK_FB_W]];
            }
        }
...
void render_init(void)
{
    ...
    for (int i = 0; i < NUM_FB; i++) {
        fbs[i] = (uint8_t *)heap_caps_malloc(DK_FB_W * DK_FB_H, MALLOC_CAP_8BIT);
