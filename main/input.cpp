 ...
 *
 * Donkey Kong is a four-way game: run left and right, climb up and down. Only the dominant
 * axis is reported, so a diagonal tilt never produces two directions, which is what the
 * four-way gate on the cabinet did mechanically.
 *
 *   tilt left / right   -> run, the direction of gravity within the panel plane
 *   tilt away / toward  -> climb, how far gravity leaves that plane
 *   BOOT button         -> jump
...
void input_update(dk_input_t *in)
{
    ...
    if (now - last_log >= 3000000) {
        last_log = now;
        ESP_LOGI(TAG, "imu %d  roll %+6.1f pitch %+6.1f -> U%d D%d L%d R%d J%d",
                 imu_ok, (double)dbg_roll, (double)dbg_pitch, in->up, in->down, in->left, in->right, in->jump);
    }
}
