/*
 * input.cpp - medal controls for Donkey Kong (held upright, like Pac-Man)
 *
 * The buttons, the power rail, the coin-then-start sequence, the mute gesture and the tilt zero
 * are all in components/medal_input, which every medal shares. What is left here is the only
 * part that is Donkey Kong's own: turning two angles into a four-way stick.
 *
 * Donkey Kong is a four-way game - run left and right, climb up and down - so only the dominant
 * axis is ever reported, which is what the four-way gate on the cabinet's stick did
 * mechanically.
 *
 *   twist left / right  -> run
 *   tip away / toward   -> climb
 *   BOOT button         -> jump; hold 3 s for sound off and on
...
void input_update(dk_input_t *in)
{
    ...
    if (now - last_log >= 3000000) {
        last_log = now;
        ESP_LOGI(TAG, "tilt %d  roll %+6.1f pitch %+6.1f -> U%d D%d L%d R%d J%d",
                 st.tilt_valid, (double)dbg_roll, (double)dbg_pitch,
                 in->up, in->down, in->left, in->right, in->jump);
    }
}
