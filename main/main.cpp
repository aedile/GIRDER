/*
 * GIRDER - Nintendo Donkey Kong (1981) on the Waveshare ESP32-C6-LCD-1.69 Fiesta medal
 */
#include "esp_log.h"
#include "esp_timer.h"
...
#include "display.h"
#include "dkong.h"
#include "dkong_roms.h"
#include "render.h"
#include "input.h"
#include "audio_hal.h"
#include "launcher_handback.h"

static const char *TAG = "GIRDER";
...
extern "C" void app_main(void)
{
    ...
    ESP_LOGI(TAG, "GIRDER starting, free heap %lu", (unsigned long)esp_get_free_heap_size());
    display_init();
    ...
    dk_roms_t roms = { to_ram(dk_rom, sizeof(dk_rom)),   to_ram(dk_snd, sizeof(dk_snd)),
                       to_ram(dk_samp, sizeof(dk_samp)), to_ram(dk_chr, sizeof(dk_chr)),
                       to_ram(dk_spr, sizeof(dk_spr)),   to_ram(dk_pal_lo, sizeof(dk_pal_lo)),
                       to_ram(dk_pal_hi, sizeof(dk_pal_hi)), to_ram(dk_colcode, sizeof(dk_colcode)) };
    dk_init(&roms);
    dk_set_dips(0x80);         /* factory: 3 lives, bonus at 7000, 1 coin 1 play */
    render_init();
    input_init();
    audio_init();
    ...
    for (;;) {
        ...
        while (owed_us >= FRAME_US) {
            ...
            if (owed_us < FRAME_US) {              /* draw only the last frame of a catch-up burst */
                ...
                if (fb) { dk_render(fb); render_submit(fb); t_submit += esp_timer_get_time() - t1; }
                else skipped++;
            } else {
                skipped++;
            }
        ...
        if (now - ldk_report >= 5000000) {
            ESP_LOGI(TAG, "5s: frames %lu drawn %lu skipped %lu dropped %lu; ms/s: emu %llu submit %llu render %llu audio %llu; heap %lu; pc %04X",
                     ...
                     (unsigned long)esp_get_free_heap_size(), dk_pc());
            frames = skipped = 0; t_emu = t_submit = t_audio = 0; ldk_report = now;
        }
    }
}
