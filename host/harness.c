 ...
 * script keys: coin start up down left right jump
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void write_ppm(const char *path, const uint8_t *fb, const uint16_t *pal)
{
    ...
    if (!f) return;
    fprintf(f, "P6\n%d %d\n255\n", DK_FB_W, DK_FB_H);
    for (int y = 0; y < DK_FB_H; y++) {
        for (int x = 0; x < DK_FB_W; x++) {
            uint16_t c = pal[fb[y * DK_FB_W + x]];
...
int main(int argc, char **argv)
{
    ...
    dk_roms_t roms = { dk_rom, dk_snd, dk_samp, dk_chr, dk_spr,
                       dk_pal_lo, dk_pal_hi, dk_colcode };
    dk_init(&roms);
    dk_set_dips((uint8_t)r10);
    dk_input_t *in = dk_input();
    ...
    static uint8_t fb[DK_FB_W * DK_FB_H];
    static int16_t abuf[4096];
    uint16_t pal[DK_PALETTE_SIZE];
    ...
    for (int f = 0; f < frames; f++) {
        ...
        for (int e = 0; e < nev; e++) {
            if (evs[e].t <= now && evs[e].t > now - 1.0 / fps) {
                ...
                else if (!strcmp(k, "jump")) in->jump = v;
            }
        }
        dk_run_frame();
        if (wav) {
            audio_acc += rate / fps; int n = (int)audio_acc; audio_acc -= n;
            dk_render_audio(abuf, n, rate); fwrite(abuf, 2, n, wav); wav_samples += n;
        }
        if (now >= next_save) {
            char path[512]; snprintf(path, sizeof(path), "%s/frame_%03d.ppm", outdir, saved);
            dk_render(fb); dk_palette(pal); write_ppm(path, fb, pal); saved++; next_save += every;
        }
        if ((f % (int)fps) == (int)fps - 1) printf("t=%ds pc=%04X snd=%03X\n", (int)(now + 1), dk_pc(), dk_snd_pc());
    }
    if (wav) {
